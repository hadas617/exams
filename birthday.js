...
function dadCake() {
  ...
  function createBalloons(num) {
    ...
    for (let i = 0; i < num; i++) {
        ...
        balloon.style.bottom = "-75px";
        balloonContainer.appendChild(balloon);
      }
    }
 createBalloons(30);
 document.getElementById('arrow').addEventListener('click',gallery);
}
...
let isHovered = false;
let currentY = 0;
const scrollSpeed = 1.01; 

const galleryMusic = new Audio("assets/music/gallery.mp3");
galleryMusic.preload = "auto";
galleryMusic.loop = true;
galleryMusic.volume = 0.3;

function gallery() {
    document.getElementById("toCard").style.display = "none";
    document.getElementById("toCardHover").style.display = "none";
    document.getElementById("toCard").style.bottom = "5vh";

    galleryMusic.currentTime = 12;
  ...
  let thumb = document.getElementById('custom-scrollbar-thumb');
  if (!scrollbar) {
    ...
    galleryContainer.appendChild(scrollbar);
  }
  const viewportHeight = galleryContainer.clientHeight;
  const trackHeight = galleryTrack.scrollHeight;
  const extraScroll = 150;  
  const maxScroll = trackHeight - viewportHeight + extraScroll;  const thumbHeight = Math.max(30, (viewportHeight / trackHeight) * viewportHeight);
  thumb.style.height = `${thumbHeight}px`;
  const maxThumbTravel = viewportHeight - thumbHeight;
  function updateScrollPositions() {
    galleryTrack.style.transform = `translate3d(0, -${currentY}px, 0)`;
    const scrollPercent = currentY / maxScroll;
    const thumbY = scrollPercent * maxThumbTravel;
    thumb.style.transform = `translate3d(0, ${thumbY}px, 0)`;
  }
  galleryContainer.addEventListener('mouseover', (e) => {
    if (e.target.tagName === 'IMG') isHovered = true;
  });
  galleryContainer.addEventListener('mouseout', (e) => {
    if (e.target.tagName === 'IMG') isHovered = false;
  });
  galleryContainer.addEventListener('wheel', (e) => {
      ...
      if (currentY >= maxScroll) {
          document.getElementById("toCard").style.display = "block";
          document.getElementById("toCardHover").style.display = "block";
      }
  }, { passive: false });
  function scrollLoop() {
  if (currentY >= maxScroll) {
  ...
  cancelAnimationFrame(scrollAnimationId);
  scrollAnimationId = requestAnimationFrame(scrollLoop);
  document.getElementById("toCard").addEventListener("click", card);
  document.getElementById("toCardHover").addEventListener("click", card);
  ...
  galleryContainer.addEventListener("touchmove", (e) => {
      ...
      currentY += distance * 0.8; // adjust sensitivity here
...
function card() {
  ...
  document.getElementById('birtdayCard').style.display = 'block';
